...
- Align loan number validation: make POST `loanSchema.loanNumber` use the same `^LN-\d+$` regex as the GET param (or relax both, but keep them consistent).
- Add CORS middleware in `backend/src/app.ts` for the upcoming React frontend (allow the frontend origin, methods, and headers).
- Decouple service from HTTP concerns: have `LoanService` return null/throw a domain error; map to 404/409 in routes instead of throwing `HTTPException` inside the service.
- Optimize duplicate check: in `craeteLoan`, call `findByLoanNumber` instead of scanning `findAll`.
- Rename `craeteLoan` to `createLoan` across service/routes for clarity.
- Make the global 500 message neutral (`"Internal Server Error"`) instead of the playful text.
- Add a short README with setup/run instructions and the `/api/v1/loans` endpoints; update `.HTTP` examples if you change validation rules.
